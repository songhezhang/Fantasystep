...
if [ ! -f /data/lib/ldap/DB_CONFIG ]; then
    ...
    ldapadd -x -D cn=admin,dc=fantasystep,dc=com -w $LDAP_MANAGER_PASSWORD -f /root/base.ldif

    ldapadd -f /root/fantasystep.ldif -D cn=config -w $LDAP_MANAGER_PASSWORD
    ldapadd -f /root/binru.ldif -D cn=admin,dc=fantasystep,dc=com -w $LDAP_MANAGER_PASSWORD

    service slapd stop
    sleep 3

    mkdir /data/lib /data/etc -p
