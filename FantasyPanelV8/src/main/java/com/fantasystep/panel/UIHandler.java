...
public class UIHandler {
	...
	private void buildLeftTree() {
		if (tree == null) {
			...
			ItemClickHandler itemh = new ItemClickHandler(tree, new NodeMenu(tree,
					tree)) {
	...
	public Window buildLoginLayout() {
		...
		loginForm.addLoginListener(new LoginForm.LoginListener(){
		    ...
		    public void onLogin(LoginForm.LoginEvent event){
				...
				try {
					UUID uuid = TreeHandler.get().authenticate(username, password);
					if (uuid != null) {
						Map<String, List<String>> headers = new HashMap<String, List<String>>();
						headers.put(EnvironmentUtil.SESSION_KEY, Collections.singletonList(uuid.toString()));
						((BindingProvider) TreeHandler.get()).getRequestContext()
								.put(MessageContext.HTTP_REQUEST_HEADERS, headers);
					}
				
					TreeHandler.updateTreeFromStorage(true);
					if (((CApplication) CApplication.getCurrent()).isDeveloperMode())
						buildDebugger();
					else
						showMainLayout();
					loginWindow.close();
				} catch (InvalidCredentialsException e) {
					Notification.show(String.format(LocalizationHandler.get(LabelUtil.LABEL_FAILED), "!"),
							LocalizationHandler.get(LabelUtil.LABEL_USER_PASSWORD_INCORRECT), Type.WARNING_MESSAGE);
				}
//		      loginForm.getWindow().removeWindow(event.getComponent().getWindow());
//		      loginForm.fireEvent(new LoginAttemptEvent(LoginViewImpl.this,accountId,password));
		    }
		  });
//		{
...
//			}
//		};
		loginWindow.setContent(LayoutUtil.addContainerLayout(loginForm,
